...
import br.com.uniplus.materialmanager.dto.request.RemocaoMaterialResponse;
...
import br.com.uniplus.materialmanager.services.MaterialService;
...
public class TurmaController {
	...
	public ModelAndView turmaBoard(@RequestParam(value = "id", required = false) Long turmaId) {
		ModelAndView model = new ModelAndView("/turma/turmas-board");
		if (turmaId != null) {
			session.setAttribute("turmaId", turmaId);
		} else {
			turmaId = (Long)session.getAttribute("turmaId");
		}
		List<MaterialResponse> materiais = materialService.getMateriais(turmaId);
		TurmaResponse turma = turmaSevice.buscaTurma(turmaId);
		
		model.addObject("materiais", materiais);
		model.addObject("turma", turma);
		return model;
	}

	@GetMapping("/turmas/material")
	public String turmaMaterial() {
		return "/turma/turmas-material";
	}
	...
	@PostMapping("/turmas/material/remove")
	public ModelAndView turmaMaterialRemover(RemocaoMaterialResponse request) {
		materialService.remove(request.getMaterialId());
		Long turmaId = (Long) session.getAttribute("turmaId");
		return turmaBoard(turmaId);
	}
